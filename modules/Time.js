function getTime() {
    var time = process.stdin.read();
    if (time !== null || time<=3600) {
        var timeSeconds = (time % 60);
        var timeMinuts = Math.round(time/60);
        console.log("Czas w minutach wynosi:  " + timeMinuts + "min i " + timeSeconds + "s.");
    } else {
        ...
        console.log("Czas w godzinach wynosi:  " + timeHours + "godz., " + timeMinuts + "min i " + timeSeconds + "s.");
	};
};
